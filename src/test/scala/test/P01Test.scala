...
class P01Test extends FlatSpec with ShouldMatchers {
  ...
  it should "return Nil if the List in Nil" in {
    val list = Nil
   ...
   it should "get the last element of a non-empty list returns 1" in {
     val list = List(1)
     P01.last(list) should equal (Some(1))
   }
   
   it should "get the last element of a non-empty list returns 3" in {
     val list = List(3)
     P01.last(list) should equal (Some(3))
   }
   ...
   //   check( Prop.forAll { list: List[Int] => (list.length > 0) ==> (P01.last(list) == Some(list.last)) } )
